...
Status: M6a candidate. This contract was frozen before any KITTI detector initialization or
prediction. M6a validates ingestion and reconstruction only.
...
- The official odometry devkit maps odometry sequence 04 to
  `2011_09_30_drive_0016`, frames 0 through 270. Its public ground-truth poses form the independent
  Tier-A oracle.
...
`avg-kitti` object store. Archive SHA256 values are recorded only after acquisition.
...
M6a downloads the canonical drive and pose-oracle drive only. The other two candidates are frozen
metadata, not detector workloads.
...
For the odometry oracle:

`T_cam0rect_from_imu = R_rect_00 @ T_cam0_from_velo @ T_velo_from_imu`.

The OXTS-derived pose is conjugated by this calibration before comparison with official camera-0
odometry poses.
...
exactly and validates the final trajectory in a common rectified-camera frame against odometry
ground truth.
