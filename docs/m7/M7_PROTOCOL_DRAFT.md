# M7 controlled history-mechanism study — protocol draft

Status: **DRAFT FOR OWNER REVIEW; NOT FROZEN; NO M7 INPUT OR DETECTOR OUTPUT EXISTS**.

This document prospectively designs M7. It does not authorize input generation, detector
initialization, inference, threshold exploration, training, tuning, or a production-policy change.
The final protocol requires a separate owner approval and freeze commit. A later input-only freeze
requires a second owner review before any new detector output may be generated.
...
F approximates H5 sweep count while spanning the long H10 physical interval. It is not guaranteed
to match E point count, pillar count, exact spacing, or density; those are measured. F is a
secondary natural-manifold comparator, not part of the 2×2 factorial and not a replacement for C.
...
sentinel's canonical corpus output, avoiding an unnecessary eleventh run. A/E are frozen and are
not rerun. Any nondeterminism stops M7 before the full corpus.
...
“near equivalent,” “essentially solved,” or new 30%/70% cutoffs are not authorized.
...
5 MiB may enter Git without explicit owner approval. This draft creates none of these artifacts.
...
- F keeps current plus ranks 2/4/6/8/10 and is a secondary unthinned comparator.
...
Owner approval of this draft would authorize a later protocol-freeze/implementation cycle only.
It would not by itself authorize input construction or detector inference.
