...
class StrictInputLedger:
    """Strictly parsed, fully ordered authorized input ledger."""
    ...
    *,
    ...
    for raw_record in conditions:
        ...
        for field in (
            ...
            _lower_hex(raw_record.get(field), length=64, name=field)
        ...
        for source in rank_sources:
            ...
            if (
                not isinstance(lag_text, str)
                or len(lag_text) != 10
                or not lag_text.startswith("0x")
            ):
        ...
        if raw_record.get("arm") == Arm.F.value and ranks != [0, 2, 4, 6, 8, 10]:
...
def load_strict_input_ledger(
    ...
    *,
    ...
    """Load the authorized file and require the complete frozen 1,712-condition schema."""
